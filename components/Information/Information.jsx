import { dateToYearOnly, getRuntime, getFallBackTitle, getMaturityRating } from '../../utils/utils';
...
function Information({ data, type }) {
  ...
  const getThreeCast =
    reduceCast.length > 3 ? `${reduceCast.slice(0, 3).join(', ')}, more` : reduceCast.join(', ');

  const genresString = genres ? genres.map((item) => item.name).join(', ') : 'Not available';
