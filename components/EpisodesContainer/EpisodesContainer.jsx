...
function EpisodesContainer({ seasons, id }) {
  ...
  const { data, error, isLoading } = useQuery([Number(id), url], () =>
    fetcher(url)
  );
  let episodes;
  ...
  return (
    <div className='px-[4%] mt-10 py-[1em] pb-0'>
      <div className='header flex justify-between'>
        <div className='font-bold text-[24px] mt-[48px] mb-[20px]'>
          Episodes
        </div>
        <Dropdown
          options={options}
      ...
      <div className='episodes_container flex flex-wrap justify-start flex-col'>
        {isLoading &&
          [...Array(selected.episode_count)].map((_, i) => (
            <div key={i} className='skeleton pl-[7.5%] flex items-center'>
              <SkeletonElement
                type='poster'
                style={{ marginBottom: 0, width: '23.5%' }}
              />
        ...
        {episodes &&
          episodes.map((episode) => (
            <EpisodePoster key={episode.id} data={episode} />
          ))}
      </div>
    </div>
  );
